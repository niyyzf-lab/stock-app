...
<BaseNode id={id} data={data} selected={selected} title="显示数值" description="显示数值内容">
    <div class="relative p-4 rounded-xl bg-white flex items-center justify-center min-w-[160px] min-h-[60px]">
        <Handle type="target" id="data-in-number" position={Position.Left} />
        <p class="text-2xl font-bold text-blue-700 text-center w-full break-words">{formatNumber(data.value as number)}</p>
